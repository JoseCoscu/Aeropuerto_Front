...
const table = document.querySelector('#workshop_table tbody')
...
const refreshPassengers = async (event) => {
  ...
  const rows = passengers
    .map((passengers) => {
      const { ship, startDate, finalDate, idReparation, time, increase, discount,status, } = passengers
      const id = { ship, startDate }
      const _dateS = startDate.substring(0, 10)
...
const get_passengers = async () => {
  try {
    const result = await fetch('http://localhost:1234/workshop', {
      method: 'GET',
      headers: {
