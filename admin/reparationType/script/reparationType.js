const deleteReparationType = async (id) => {
    const response = fetch(`http://localhost:1234/reparationType/${id}`, {
      method: 'DELETE'
    })
    return response.ok
  }
  
  const installationsType_table = document.querySelector('#reparationType_table')
  installationsType_table.addEventListener('click', async (event) => {
    if (event.target.type === 'submit') {
      ...
      if (event.target.classList.contains('edit_button')) {
        ...
        const ventanaPopup = window.open('', 'Formulario Tipo de Reparaciones', 'width=600,height=800')
        ...
        temporal.querySelector('#idReparationType').setAttribute('value', id_installationsType)
      ...
      if (event.target.classList.contains('delete_button')) {
        ...
        const response = await deleteReparationType(id_clientType)
        location.reload()
